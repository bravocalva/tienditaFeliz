...
pregunta(genero_nortena, '¿El artista se dedica a la música norteña?').
pregunta(genero_banda, '¿El artista se dedica a la música de banda?').
...
respuesta(si, 1).
respuesta(ps, 1). % Aumentamos la puntuación de "Probablemente sí"
respuesta(ns, 0).
respuesta(pn, -0.5). % Reducimos la penalización de "Probablemente no"
respuesta(no, -1).
...
suposicion_artista(_) :-
    write('No se puede determinar el artista con la información proporcionada.'), nl,
    retractall(conocimiento(_, _)).

% Hechos sobre características de los artistas
